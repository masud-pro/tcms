    ...
    <div class="btn-group mb-4 mt-1 mr-3" wire:ignore.self>
        ...
        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton" wire:ignore.self >
            <div class="px-8 sm-hight">
                <input wire:model.prevent="search" name="name" class="form-control sm-hight" id="name" type="text">
            </div>


                @foreach ($filteredBatches as $batch)
                    <li>
                        <a class="dropdown-item pt-2" href="{{ route('course.attendance.create', $batch->id) }}">{{ $batch->name }}</a>
                    </li>
                @endforeach
